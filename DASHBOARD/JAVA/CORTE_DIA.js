...
async function cargarDatosDelDia() {
    try {
        ...
        const response = await fetch(`OBTENER_VENTAS_DIA.php?fecha=${hoy}`, {
            headers: {
        ...
        if (data.success) {
            actualizarResumenVentas(data.data.totales);
            actualizarTablaVentas(data.data.ventas);
            if (data.meta.tabla_retiros_existe) {
                actualizarTablaRetiros(data.data.retiros);
            }
        } else {
...
// Función para actualizar tabla de retiros
...
async function realizarRetiro() {
    ...
    try {
        ...
        const response = await fetch('GUARDAR_RETIROS.php', {
            method: 'POST', // Esto es crítico
            headers: {
            ...
            body: JSON.stringify({ // Convierte los datos a JSON
                motivo: motivo,
                monto: monto,
                id_empleado: 1, // Cambia por el ID real
                notas: document.getElementById('notas-retiro')?.value || ''
            })
        });
...
// Función para registrar venta desde la interfaz de caja
    ...
    try {
        const response = await fetch('GUARDAR_VENTA.php', {
            ...
            body: JSON.stringify({
                ...
                id_empleado: ventaData.idEmpleado || 1,
                id_cliente: ventaData.idCliente || null,
                concepto: ventaData.concepto || 'Venta normal'
            })
        });
        ...
        // Recargar datos después de registrar venta
        cargarDatosDelDia();
        return result;
    } catch (error) {
